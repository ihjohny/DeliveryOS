...
async function runVerification() {
  ...
  // 4. Verify Directory Structure (apps/admin_portal)
  console.log('\n📁 4. Verifying Directory Invariants (apps/admin_portal)...');
  const projectRoot = path.resolve(rootDir, '../..');
  const adminPortalPath = path.join(projectRoot, 'apps/admin_portal');
  assert(fs.existsSync(adminPortalPath), 'Directory apps/admin_portal exists');
  const pkgJsonPath = path.join(adminPortalPath, 'package.json');
  assert(fs.existsSync(pkgJsonPath), 'apps/admin_portal/package.json exists and is valid');
